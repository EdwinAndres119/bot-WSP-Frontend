...
- `GET /api/status` — pensado para poll cada 1-2s; `state` es `idle | starting | qr | extracting | completed | error`; trae `qrDataUrl` (data URL de imagen) solo en `state === 'qr'`, y `progress` (`chatsFound/processed/failed/saved/failedChats/emptyChats`) durante `extracting` — `failedChats` es `{chatId, chatName, error}[]` (error técnico real), `emptyChats` es `{chatId, chatName, reason}[]` (se revisó bien, no había mensajes; `reason` es uno de tres textos fijos del backend)
